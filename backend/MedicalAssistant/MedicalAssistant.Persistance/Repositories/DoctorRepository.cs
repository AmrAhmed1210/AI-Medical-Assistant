...
public class DoctorRepository : GenericRepository<Doctor>, IDoctorRepository
{
    ...
    public Task<IEnumerable<Doctor>> GetUnavailableDoctorsAsync()
    {
        // حالياً نعتبر الطبيب غير متاح لو معندوش مواعيد عمل (Availability)
        var doctorIdsWithSlots = _context.Set<DoctorAvailability>().Select(a => a.DoctorId).Distinct();
        var query = _dbSet.Where(d => !doctorIdsWithSlots.Contains(d.Id)).AsEnumerable();
