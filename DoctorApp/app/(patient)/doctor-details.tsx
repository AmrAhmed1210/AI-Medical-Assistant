...
import { getDoctorById, getReviewsByDoctor, addReview, DoctorDetails, Review } from "../../services/doctorService";
...
export default function DoctorDetailsScreen() {
  ...
  const submitReview = async () => {
    ...
    try {
      const newReview = await addReview(Number(doctorId), myRating, myComment.trim());
        ...
        <View style={styles.profileCard}>
          ...
          <View style={styles.statsRow}>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statVal}>{doctor.reviewCount}</Text>
              ...
              <View key={i} style={styles.reviewCard}>
                <View style={styles.reviewTop}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.reviewDate}>{new Date(r.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</Text>
                  </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.overlay}>
          <View style={[styles.sheet, { paddingBottom: 36 }]}>
            ...
            <View style={[styles.inputRow, cardErrors.cardNumber && styles.inputError]}>
              ...
              <TextInput style={[styles.input, { flex: 1, borderWidth: 0, marginBottom: 0, padding: 0 }]} placeholder="0000 0000 0000 0000" placeholderTextColor="#BBB" value={cardNumber} onChangeText={(v) => setCardNumber(formatCardNumber(v))} keyboardType="numeric" maxLength={19} />
            </View>
            ...
            <View style={{ flexDirection: "row", gap: 12 }}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={[styles.input, cardErrors.expiry && styles.inputError]} placeholder="MM/YY" placeholderTextColor="#BBB" value={expiry} onChangeText={(v) => setExpiry(formatExpiry(v))} keyboardType="numeric" maxLength={5} />
                {cardErrors.expiry && <Text style={styles.fieldError}>{cardErrors.expiry}</Text>}
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.fieldLabel}>CVV</Text>
                <TextInput style={[styles.input, cardErrors.cvv && styles.inputError]} placeholder="•••" placeholderTextColor="#BBB" value={cvv} onChangeText={(v) => setCvv(v.replace(/\D/g, "").slice(0, 4))} keyboardType="numeric" secureTextEntry maxLength={4} />
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.overlay}>
          <View style={[styles.sheet, { paddingBottom: 36 }]}>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
              <TouchableOpacity style={[styles.confirmPayBtn, { flex: 1, backgroundColor: "#F0F0F0" }]} onPress={() => setShowAddReview(false)}>
                <Text style={[styles.confirmPayTxt, { color: "#555" }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.confirmPayBtn, { flex: 2 }]} onPress={submitReview} disabled={savingReview}>
                {savingReview ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.confirmPayTxt}>Submit Review</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </KeyboardAvoidingView>
          ...
          <View style={styles.successCard}>
            ...
            <View style={styles.successDetails}>
              ...
              <View style={styles.successRow}>
                ...
                <Text style={styles.successDetailTxt}>{payMethod === "visa" ? "Paid by Visa" : "Pay on Arrival"}</Text>
              </View>
            </View>
