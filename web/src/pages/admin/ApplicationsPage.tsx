            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-4 text-sm font-semibold transition-colors border-b-2 ${
                activeTab === tab.value
                  ...
                  : 'border-transparent text-gray-500 dark:text-slate-450 hover:text-gray-700 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-slate-800'
              }`}
            >
        ...
        <div className="divide-y divide-gray-50 dark:divide-slate-800/60">
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400 dark:text-slate-500 text-sm">No {activeTab.toLowerCase()} applications</p>
            </div>
          ...
          >
            ...
            >
              ...
              <textarea
                ...
                className="w-full text-sm bg-white dark:bg-slate-950 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 border border-gray-200 dark:border-slate-800 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-450/30 shadow-sm resize-none"
              />
...
function ApplicationRow({
  ...
  return (
    <div className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
      {/* Summary row */}
      <div className="flex items-center gap-4 px-5 py-4 cursor-pointer" onClick={onToggle}>
        {app.photoUrl ? (
          <img src={app.photoUrl} alt={app.name} className="w-10 h-10 rounded-full object-cover shadow border border-gray-100 dark:border-slate-800 flex-shrink-0" />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-gray-800 dark:text-slate-100">{app.name}</p>
          <p className="text-xs text-gray-500 dark:text-slate-400">{app.email} · {app.specialtyName}</p>
        </div>
        ...
        {isExpanded ? <ChevronUp size={16} className="text-gray-400 dark:text-slate-500 flex-shrink-0" /> : <ChevronDown size={16} className="text-gray-400 dark:text-slate-500 flex-shrink-0" />}
      </div>
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="px-5 pb-5 space-y-4 border-t border-gray-100 dark:border-slate-800/80 pt-4">
              ...
              {app.processedAt && (
                <p className="text-xs text-gray-400 dark:text-slate-500">Processed: {new Date(app.processedAt).toLocaleString()}</p>
              )}
...
function InfoCell({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="bg-gray-50 dark:bg-slate-950/40 rounded-xl px-3 py-2.5">
      <p className="text-xs text-gray-400 dark:text-slate-500 flex items-center gap-1 mb-0.5">{icon}{label}</p>
      <p className="text-sm font-medium text-gray-700 dark:text-slate-350">{value}</p>
    </div>
  )
}
