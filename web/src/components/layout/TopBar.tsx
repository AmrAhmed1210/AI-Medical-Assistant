      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative hidden md:block group">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="pl-9 pr-4 py-2 text-sm rounded-xl w-64 focus:outline-none focus:ring-2 focus:ring-primary-500/30 transition-all bg-slate-100 dark:bg-slate-800/50 border border-transparent dark:border-slate-700/50 text-slate-700 dark:text-slate-200"
          />
        </div>
          ...
          <AnimatePresence>
            {showNotif && (
              <motion.div
                ...
                className="absolute right-0 top-full mt-3 w-80 rounded-2xl overflow-hidden z-50 glass-card dark:bg-slate-900 shadow-2xl dark:shadow-none border border-slate-200 dark:border-slate-800"
              >
                ...
                <div className="max-h-72 overflow-y-auto scrollbar-hide">
                  {notifications.length === 0 ? (
                    <p className="text-center text-sm py-8 text-slate-400 dark:text-slate-500">No notifications</p>
                  ) : notifications.map((n) => (
                    <div
                      key={n.id}
                      onClick={() => removeNotification(n.id)}
                      className="w-full text-left flex items-start gap-3 p-3 cursor-pointer transition-colors border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/50"
                    >
                      <div className={cn(
        ...
        {/* User Profile */}
        <div className="flex items-center gap-3 pl-5 ml-2 border-l border-slate-200 dark:border-slate-700/50">
